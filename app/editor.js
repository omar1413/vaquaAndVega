...
VAQUA.craeteJsonFile = function () {
    var dados = {"data": {"url": "data/omar.json"},
        "mark": "area",
        "encoding": {
            "x": {
...
ved.uri = function (entry) {
    console.log(ved.path + 'spec/' + ved.currentMode + '/' + entry.name + '.json');

    VAQUA.craeteJsonFile();
...
ved.parseVg = function (callback) {
    ...
    //opt.data[0].url = "data/movies.json";
